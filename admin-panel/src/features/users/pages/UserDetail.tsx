        ...
        <Col xs={24} lg={8}>
          <Card title={t("userDetail.basicInfo")}>
            <ProDescriptions column={1} size="small" style={{ margin: 0 }}>
              <ProDescriptions.Item label={t("users.username")}>
                {user.username}
              </ProDescriptions.Item>
              <ProDescriptions.Item label={t("userDetail.slug")}>
                <CopyCell value={user.slug} />
              </ProDescriptions.Item>
              <ProDescriptions.Item label={t("users.email")}>
                {user.email}
              </ProDescriptions.Item>
              ...
              <ProDescriptions.Item label={t("users.registerMethod")}>
                {user.registerMethod}
              </ProDescriptions.Item>
              <ProDescriptions.Item label={t("userDetail.status")}>
                {user.status}
              </ProDescriptions.Item>
              <ProDescriptions.Item label={t("users.registered")}>
              ...
              <div style={{ marginTop: 16 }}>
                ...
                >
                  {user.profile.displayName && (
                    <ProDescriptions.Item label={t("userDetail.displayName")}>
                      {user.profile.displayName}
                    </ProDescriptions.Item>
                  )}
                  ...
                  {user.profile.bio && (
                    <ProDescriptions.Item label={t("userDetail.bio")}>
                      {user.profile.bio}
                    </ProDescriptions.Item>
                  )}
                </ProDescriptions>
        ...
        <Col xs={24} lg={8}>
          <Card title={t("userDetail.storage")}>
            <ProDescriptions column={1} size="small">
              <ProDescriptions.Item label={t("userDetail.used")}>
                {formatBytes(user.usedBytes)}
              </ProDescriptions.Item>
              <ProDescriptions.Item label={t("userDetail.base")}>
                {formatBytes(user.baseBytes)}
              </ProDescriptions.Item>
              <ProDescriptions.Item label={t("userDetail.memberBonus")}>
                {formatBytes(user.memberBonusBytes)}
              </ProDescriptions.Item>
              <ProDescriptions.Item label={t("userDetail.pack")}>
                {formatBytes(user.packBytes)}
              </ProDescriptions.Item>
              <ProDescriptions.Item label={t("userDetail.total")}>
