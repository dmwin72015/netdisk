<script lang="ts">
  ...
  async function handleDelete(u: AdminUser) {
    const ok = await confirmDelete(
      m.admin_confirm_delete_user({ name: u.username }),
    );
    if (!ok) return;
    try {
  ...
  <div class="flex flex-wrap items-center gap-3">
    <div class="relative flex-1 min-w-[200px] max-w-xs">
      ...
      <input
        ...
        class="w-full rounded-lg border border-line bg-surface py-2 pl-9 pr-8 text-sm text-ink placeholder:text-ink-4 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
      />
      {#if searchQuery}
        <button
          type="button"
    ...
    <Select.Root type="single" bind:value={roleFilter}>
      <Select.Trigger
        class="flex items-center justify-between gap-2 rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink-3 min-w-[130px] data-[placeholder]:text-ink-4 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
      >
        ...
        {:else}
          {#each users as u (u.id)}
            <tr class="transition-colors hover:bg-surface-sunken">
              <td class="truncate px-4 py-3 font-medium text-ink"
                >{u.username}</td
              >
              <td class="truncate px-4 py-3 text-ink-3" title={u.email}
                >{u.email}</td
              >
              <td class="truncate px-4 py-3 text-xs text-ink-4"
                >{u.registerMethod}</td
              >
              <td class="px-4 py-3">
                <span
                  class="rounded px-2 py-0.5 text-xs font-medium {u.role ===
                  'admin'
                    ? 'bg-primary-soft text-primary'
                    : 'bg-surface-sunken text-ink-3'}">{u.role}</span
                >
              </td>
              <td class="px-4 py-3">
                {#if u.totalBytes > 0}
                  ...
                  <div class="mb-1 flex items-center justify-between text-xs">
                    <span class="text-ink-3"
                      >{fmtSize(u.usedBytes)} / {fmtSize(u.totalBytes)}</span
                    >
              ...
              <td class="px-4 py-3 text-xs text-ink-4"
                >{fmtDate(u.createdAt)}</td
              >
              <td class="px-4 py-3">
...
<Dialog
  ...
  title={m.admin_create_user()}
  onConfirm={handleCreateUser}
  confirmText={creating ? m.admin_creating() : m.admin_create_user()}
  showCancel
  size="sm"
>
    ...
    <div class="space-y-4">
      <div class="relative">
        ...
        <input
          ...
          class="w-full rounded-lg border border-line bg-surface px-3 py-2 pr-8 text-sm text-ink placeholder:text-ink-4 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
        />
        {#if createUsername}
          <button
            type="button"
      ...
      <div>
        ...
        <Select.Root type="single" bind:value={createRole}>
          ...
          <Select.Portal>
            <Select.Content
              class="z-[60] overflow-hidden rounded-lg border border-line bg-surface p-1 shadow-pop"
              sideOffset={4}
              side="top"
              align="start"
            >
    ...
    <div class="space-y-4">
      <div>
        ...
        <Select.Root type="single" bind:value={editRoleValue}>
          ...
          <Select.Content
            class="z-50 w-[var(--bits-select-trigger-width)] overflow-hidden rounded-lg border border-line bg-surface p-1 shadow-pop"
            sideOffset={4}
            align="start"
          >
            {#each roleOptions as opt}
              ...
              >
                {opt.label}
                {#if opt.value === editRoleValue}<Check
                    size={14}
                    class="ml-auto"
                  />{/if}
              </Select.Item>
