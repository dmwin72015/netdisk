<script lang="ts">
	...
	import { ArrowLeft, LoaderCircle, CircleAlert, Clock, Info } from '@lucide/svelte';
		...
		{#if item.status === 'done'}
			<div class="group relative overflow-hidden rounded-2xl bg-black">
				...
				<button type="button" onclick={() => showStats = !showStats}
					class="absolute top-3 right-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-black/50 text-white opacity-0 backdrop-blur-sm transition-opacity hover:bg-black/70 group-hover:opacity-100"
					title="Stats (S)">
					<Info size={16} />
				</button>
			</div>
