...
{#if wrap}
	<div role="region" aria-label={ariaLabel} class="bg-surface font-mono text-sm text-ink-2">
		{#each rows as row (row.key)}
			<div class="flex items-start" style={`min-height: ${ROW_HEIGHT}px; line-height: ${ROW_HEIGHT}px;`}>
				<span class="sticky left-0 z-10 w-16 shrink-0 select-none border-r border-line-soft bg-surface-sunken px-3 text-right text-xs text-ink-4">
...
{:else}
	<div
		...
		class="max-h-[80vh] overflow-auto bg-surface font-mono text-sm text-ink-2"
	>
