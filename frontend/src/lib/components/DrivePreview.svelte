<script lang="ts">
  import { Copy, Download, TextWrap } from "@lucide/svelte";
  ...
  let formattedTextContent = $derived(
    textContent === null ? null : formatTextContent(textContent),
  );
  ...
  function formatTextContent(content: string) {
    if (textTruncated || !isJson || content.length > JSON_FORMAT_LIMIT)
      return content;
    try {
  ...
  async function readTextPreview(
    ...
    try {
      while (true) {
        ...
        if (
          limited.truncated ||
          content.length >= LARGE_TEXT_MAX_PREVIEW_CHARS
        ) {
          content = limited.truncated
            ? limited.content
            : content.slice(0, LARGE_TEXT_MAX_PREVIEW_CHARS);
          truncated = true;
          await reader.cancel();
          break;
        }
  ...
  function limitTextLines(content: string): TextPreviewResult {
    ...
    if (lines.length <= LARGE_TEXT_PREVIEW_LINES)
    ...
    {:else if blobUrl}
      <div class="flex items-center justify-center p-4">
        <img
          ...
          alt={name}
          loading="lazy"
          class="max-h-[75vh] rounded-lg object-contain"
        />
      </div>
    {/if}
  {:else if isVideo}
    ...
    {:else if formattedTextContent !== null}
      ...
      >
        <button
          ...
          class="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs transition-colors {textWrap
        ...
        >
          <TextWrap size={14} />
          <span>
            {textWrap ? m.text_preview_no_wrap() : m.text_preview_wrap()}
          </span>
        </button>
      </div>
      {#if textTruncated}
        <div
          class="border-b border-warning bg-warning-soft px-5 py-2 text-xs text-warning"
        >
          {m.text_preview_truncated({
            lines: String(LARGE_TEXT_PREVIEW_LINES),
          })}
        </div>
      {/if}
      <VirtualTextViewer
