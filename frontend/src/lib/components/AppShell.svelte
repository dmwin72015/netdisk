...
<div class="bg-surface-muted text-ink flex h-screen">
	<!-- Sidebar -->
	<aside class="border-line bg-surface sticky top-0 flex h-screen w-16 shrink-0 flex-col items-center border-r py-4">
		<!-- Logo (flat at rest — no glow) -->
		<a href="/" class="mb-6 flex items-center justify-center" aria-label="NetDisk">
			<span class="bg-primary text-primary-on flex h-9 w-9 items-center justify-center rounded-xl">
		...
		>
			{#snippet trigger()}
				{#if $user?.profile?.avatarUrl}
					...
					/>
				{:else}
					<span class="bg-primary-soft text-primary flex h-7 w-7 items-center justify-center rounded-full">
			...
			<div class="border-line-soft border-b px-3 py-2 mb-1">
				<p class="text-ink truncate text-sm font-medium" title={$user?.profile?.displayName || $user?.username || ''}>
	...
	<div class="flex min-w-0 flex-1 flex-col">
		<header class="border-line bg-surface/85 sticky top-0 z-30 flex h-14 items-center gap-3 border-b px-6 backdrop-blur">
			<h1 class="text-ink min-w-[8rem] text-[15px] font-semibold tracking-tight">{pageTitle}</h1>
			...
			<button
				...
				class="border-line bg-surface-sunken text-ink-3 hover:border-ink-5 hover:bg-surface-muted hover:text-ink-2 ml-2 hidden h-9 max-w-xl flex-1 items-center gap-2 rounded-lg border px-3 text-sm transition-colors duration-150 md:flex"
			>
				<Search size={15} strokeWidth={1.75} />
				<span class="flex-1 text-left">{m.search_files()}…</span>
				<kbd class="border-line bg-surface text-ink-3 rounded border px-1.5 py-0.5 text-[10px] font-medium">
					⌘K
				</kbd>
			</button>
		...
		<main class="min-h-0 min-w-0 flex-1 overflow-y-auto px-6 py-5">
			{@render children()}
		</main>
	</div>
</div>
