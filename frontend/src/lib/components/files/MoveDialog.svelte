<script lang="ts">
  ...
  import emptyFolderSvg from "$lib/assets/empty-states/empty-folder.svg";
  ...
  let selectableDirs = $derived(
    dirs.filter((dir) => !excludedIds.includes(dir.slug)),
  );
  ...
  let targetName = $derived(
    selectedSlug
      ? (currentDir?.slug === selectedSlug
          ? currentDir.name
          : selectableDirs.find((dir) => dir.slug === selectedSlug)
              ?.fileName) || m.all_files()
      : m.all_files(),
  );
...
<Dialog
  ...
  closable={!submitting}
  size="lg"
  bodyClass="h-[68vh] p-0 flex flex-col min-h-0 overflow-hidden"
>
  <div class="flex items-center gap-2 border-b border-line-soft px-5 py-2.5">
    ...
    <div class="min-w-0 flex-1 truncate text-xs text-ink-3">
      <button
        type="button"
        class="text-ink-3 hover:text-primary"
        onclick={() => {
          path = [];
          selectedSlug = "";
          void fetchDirs(null, 1, false);
  ...
  <div class="border-b border-line-soft px-4 py-2">
    ...
    >
      <span
        class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-surface-sunken text-ink-3"
      >
        <FolderOpen size={18} />
      </span>
  ...
  <div class="flex-1 overflow-y-auto p-4">
    {#if error}
      <div
        class="mb-3 rounded-lg border border-danger bg-danger-soft px-3 py-2 text-sm text-danger"
      >
        {error}
      </div>
    {/if}
    ...
    {:else if selectableDirs.length === 0}
      <div class="flex flex-col items-center justify-center py-14 text-center">
        <img src={emptyFolderSvg} class="mb-2 w-28 h-28" alt="" />
      ...
      <div class="space-y-1">
        {#each selectableDirs as dir (dir.slug)}
          ...
          >
            ...
            >
              <span
                class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary-soft text-primary"
              >
                <Folder size={18} />
              </span>
  ...
  <div
    class="flex items-center justify-between gap-3 border-t border-line-soft px-5 py-3"
  >
