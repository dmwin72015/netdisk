...
>
 ...
 {:else}
 <!-- Stats bar -->
 <div class="flex items-center justify-between border-b border-line-soft px-5 py-2.5 text-xs text-ink-3">
 <button
 type="button"
 onclick={toggleAll}
 class="flex items-center gap-1.5 text-sm text-ink-3 hover:text-ink"
 >
 <span class="flex h-4 w-4 items-center justify-center rounded border {selected.size === files.length ? 'border-primary bg-primary text-white' : 'border-line'}">
 {#if selected.size === files.length}
 <Check size={12} />
 {/if}
 </span>
 ...
 >
 <span class="flex h-4 w-4 shrink-0 items-center justify-center rounded border {selected.has(i) ? 'border-primary bg-primary text-white' : 'border-line'}">
 {#if selected.has(i)}
 <Check size={12} />
 {/if}
 </span>
 ...
 <span class="shrink-0 text-xs text-ink-4">{fmtSize(item.file.size)}</span>
 </button>
 {/each}
 </div>
