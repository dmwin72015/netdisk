...
<Dialog.Root bind:open onOpenChange={(v) => { if (!v) closeDialog(); }}>
 ...
 />
 {#if query}
 <button type="button" onclick={clearQuery} class="rounded-lg p-1 text-ink-4 transition-colors hover:bg-surface-sunken hover:text-ink-3">
 <X size={16} />
 </button>
 {/if}
 </div>
 ...
 class="rounded-lg px-2.5 py-1 text-xs font-medium transition-colors {selectedCategory === cat.key ? 'bg-primary text-white' : 'bg-surface-sunken text-ink-3 hover:bg-line hover:text-ink-2'}"
 >
 {cat.label}
 </button>
 {/each}
 ...
 class="flex w-full items-center gap-3 px-4 py-2.5 text-left transition-colors {isHighlighted ? 'bg-primary-soft' : 'hover:bg-surface-sunken'}"
 >
 ...
 <span class="shrink-0 rounded bg-surface-sunken px-1.5 py-0.5 text-[10px] font-medium uppercase text-ink-3">{item.fileCategory || '—'}</span>
 </button>
 {/each}
 {#if results.length < total}
 <div class="border-t border-line-soft px-4 py-2 text-center text-xs text-ink-4">
 {m.total_items({ total: String(total) })} · {m.search_files()} {m.load_more()}
 </div>
 {/if}
 {:else if query.trim() || selectedCategory}
 <div class="py-12 text-center text-sm text-ink-4">{m.no_files()}</div>
 {:else}
 <div class="py-12 text-center text-sm text-ink-4">{m.search_files()}</div>
 {/if}
 </div>

 <!-- Footer hint -->
 <div class="flex items-center gap-3 border-t border-line-soft px-4 py-2 text-[11px] text-ink-4">
 <span><kbd class="rounded border border-line px-1 py-0.5 text-[10px] text-ink-3">↑↓</kbd> {m.sort_by()}</span>
