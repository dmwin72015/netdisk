...
import { fmtSize, getDownloadUrl, getPreviewUrl, listDrive, driveCheckUpload, initDriveUpload } from './drive';
...
describe('url helpers', () => {
	beforeEach(() => {
		vi.stubGlobal('localStorage', {
			getItem: (k: string) => (k === 'vf.access' ? 'tok123' : null),
		});
	});
...
describe('listDrive', () => {
	...
	it('includes q and parentId when provided', async () => {
		...
		expect(url).toContain('parent_id=dir-123');
	});
});
...
describe('driveCheckUpload', () => {
	...
	it('returns full status with file_id', async () => {
		const fetchSpy = vi.fn().mockResolvedValue(
			new Response(JSON.stringify({ data: { status: 'full', file_id: 'file-1' } }), {
				status: 200,
				headers: { 'Content-Type': 'application/json' },
			})
		);
...
describe('initDriveUpload', () => {
	it('includes sha256 in request body', async () => {
		...
		expect(body.total_size).toBe(1024);
	});
