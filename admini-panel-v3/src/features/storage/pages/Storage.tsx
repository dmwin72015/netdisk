...
const CATEGORY_COLORS: Record<string, string> = {
  video: '#722ed1', audio: '#13c2c2', image: '#1890ff',
  document: '#52c41a', archive: '#fa8c16', other: '#eb2f96', trash: '#ff4d4f',
};
const CATEGORY_LABELS: Record<string, string> = {
  video: '视频', audio: '音频', image: '图片',
  document: '文档', archive: '压缩包', other: '其他', trash: '回收站',
};
...
export default function StoragePage() {
  ...
  if (isLoading) {
    return <div className="text-center p-15"><Spin size="large" /></div>;
  }
  if (error) {
    return <PageContainer><Result status="error" title="加载失败" subTitle={error.message} /></PageContainer>;
  }
  if (!categories || categories.length === 0) {
    return <PageContainer><Result status="warning" title="暂无数据" /></PageContainer>;
  }
  ...
  return (
    <PageContainer title="存储概览">
        ...
        {regularCategories.map(stat => {
          ...
          return (
            <Col xs={24} sm={12} lg={8} key={stat.category}>
              <Card hoverable title={CATEGORY_LABELS[stat.category] || stat.category}>
                ...
                <div className="text-right text-xs text-[#999] mt-1">占比 {pct}%</div>
              </Card>
            </Col>
          );
        })}
      ...
      {trashCategory && (
        <Card title="回收站" className="mt-6">
          <Row gutter={16} align="middle">
            <Col>
              <span className="text-lg font-semibold text-[#ff4d4f]">{formatBytes(trashCategory.bytes)}</span>
            </Col>
            <Col><span className="text-[#999]">{trashCategory.count} 个文件</span></Col>
            <Col flex="auto">
              <Progress
                percent={totalBytes > 0 ? Math.round((trashCategory.bytes / totalBytes) * 100) : 0}
                strokeColor={CATEGORY_COLORS.trash} size="small"
              />
            </Col>
          </Row>
        </Card>
