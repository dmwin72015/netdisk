...
import { Button, Card, Col, Result, Row, Spin, Statistic, Table, Tag } from 'antd';
import type { ColumnsType } from 'antd/es/table';
import { ArrowLeftOutlined } from '@ant-design/icons';
import { ProDescriptions } from '@ant-design/pro-components';
import { PageContainer } from '@/components/PageContainer';
import CopyCell from '@/components/CopyCell';
import { usePhysicalFileDetail } from '@/api/admin.hooks';
...
export default function PhysicalFileDetailPage() {
  ...
  const userFileColumns: ColumnsType<CleanupQueryUserFile> = [
    ...
    { title: '文件名', dataIndex: 'fileName', width: 200, ellipsis: true },
    {
      ...
      key: 'user',
      width: 120,
      render: (_, record) => <Link to={`/users/${record.userId}`}>{record.username}</Link>,
    },
  ...
  return (
    ...
    >
      <Row gutter={16} className="mb-6">
        <Col span={6}><Card><Statistic title="总上传数" value={totalUploads} /></Card></Col>
        <Col span={6}><Card><Statistic title="独立用户数" value={uniqueUsers} /></Card></Col>
        <Col span={6}><Card><Statistic title="总大小" value={formatBytes(totalSize)} /></Card></Col>
        <Col span={6}><Card><Statistic title="引用次数" value={totalUploads} /></Card></Col>
      </Row>

      <Card title="物理文件信息" className="mb-6">
        <ProDescriptions column={2} bordered size="small">
          <ProDescriptions.Item label="ID">{physicalFile.id}</ProDescriptions.Item>
          <ProDescriptions.Item label="Slug"><CopyCell value={physicalFile.slug} /></ProDescriptions.Item>
          ...
          <ProDescriptions.Item label="文件大小">{formatBytes(physicalFile.fileSize)}</ProDescriptions.Item>
          <ProDescriptions.Item label="MIME 类型">{physicalFile.mimeType}</ProDescriptions.Item>
          ...
          <ProDescriptions.Item label="哈希算法">{physicalFile.hashAlgo}</ProDescriptions.Item>
          <ProDescriptions.Item label="状态">
            <Tag color={STATUS_COLORS[physicalFile.status] || 'default'}>{physicalFile.status}</Tag>
          </ProDescriptions.Item>
          <ProDescriptions.Item label="创建时间">{formatDate(physicalFile.createdAt)}</ProDescriptions.Item>
