...
type GetOAuthAccountsByUserIDRow struct {
	ID                int64              `json:"id"`
	UserID            int64              `json:"userId"`
	Provider          string             `json:"provider"`
	ProviderAccountID string             `json:"providerAccountId"`
...
func (q *Queries) GetOAuthAccountsByUserID(ctx context.Context, userID int64) ([]GetOAuthAccountsByUserIDRow, error) {
	rows, err := q.db.Query(ctx, getOAuthAccountsByUserID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetOAuthAccountsByUserIDRow
	for rows.Next() {
		var i GetOAuthAccountsByUserIDRow
		if err := rows.Scan(
			&i.ID,
			&i.UserID,
			&i.Provider,
